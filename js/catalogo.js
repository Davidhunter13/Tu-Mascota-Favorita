...
function mostrarPorArea(area) {
  ...
  document.addEventListener('keyup', teclas => {
    ...
    if (teclas.target.matches('#Buscador')) {
        ...
        document.querySelectorAll('.targetas').forEach(targetas => {

          targetas.querySelector('h2').textContent.toLowerCase().includes(teclas.target.value.toLowerCase())
            ? targetas.classList.remove('filtro')
            : targetas.classList.add('filtro')
